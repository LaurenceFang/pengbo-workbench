...
class StrategyService:
    ...
    def _render_backtest_markdown(self, backtest: StrategyBacktestResponse) -> str:
        ...
        lines = [
            ...
            "- Live orders: `none`",
            "- Evidence pack: `strategy_backtest`",
            ...
            "",
            f"- Generated source: `strategy backtest {backtest.run_id}`",
            f"- Data freshness: `{'degraded' if backtest.diagnostics.degraded_symbols else 'observed'}`",
            f"- Provider status: `factor snapshot {backtest.factor_run_id}`",
            f"- Audit references: `{len(execution_refs)}` linked execution intent(s)",
            f"- Evidence quality: `simulated; no live orders`",
            "",
            "## Factor Evidence",
            "",
    ...
    def _render_paper_markdown(self, session: StrategyPaperSessionResponse) -> str:
        ...
        lines = [
            ...
            "- Live orders: `none`",
            "- Evidence pack: `strategy_paper_session`",
            "- Private-state boundary: `no credentials, Stronghold vaults, unlock secrets, session tokens, or runtime databases are included`",
            "",
            "## Evidence Pack Summary",
            "",
            f"- Generated source: `paper session {session.session_id}`",
            f"- Data freshness: `{'degraded' if session.diagnostics.degraded_symbols else 'observed'}`",
            f"- Provider status: `linked backtest {session.backtest_run_id}`",
            f"- Audit references: `{len(execution_refs)}` linked execution intent(s)",
            f"- Evidence quality: `simulated paper ledger; no live orders`",
            "",
            "## Ledger",
            "",
        ]
