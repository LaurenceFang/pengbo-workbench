...
const SECTIONS = [
  ...
  },
  {
    ...
    items: [
      "Dashboard 的 AI Control 是全局入口；默认关闭，保存为本地模式或云端模式后，Research 页才会生成。",
      "本地模式默认指向 Ollama 接口，生成内容只能引用当前 Research brief、结构化证据、provenance 和 data-quality 边界。",
      "云端模式在 Dashboard 选择 ChatGPT、Gemini、Grok、Claude、DeepSeek、Qwen 或自定义接口，但单次生成仍必须在 Research 页确认脱敏预览。",
      "云端密钥只读取本机环境变量 PENGBO_AI_CLOUD_API_KEY；未确认、预览过期或缺少本机 API key 时会 blocked。",
