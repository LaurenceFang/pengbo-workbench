...
const outputDir = path.join(repoRoot, "logs", "visual-design-screenshots");
const vitePort = Number(process.env.PENGBO_VISUAL_PORT ?? 4174);
...
async function captureViewport(browser, viewport) {
  ...
  for (const target of pages) {
    ...
    const screenshotName = `${viewport.name}-${target.key}.png`;
    const screenshotPath = path.join(outputDir, screenshotName);

    if (!workspaceBox || workspaceBox.width < 600 || workspaceBox.height < 500) {
      failures.push(`${viewport.name}/${target.label}: workspace is too small or missing`);
    }
    if (!sidebarBox || sidebarBox.width < 220) {
      failures.push(`${viewport.name}/${target.label}: sidebar is too small or missing`);
    }
    if (!activeTitle || activeTitle.length < 3) {
      failures.push(`${viewport.name}/${target.label}: topbar title did not render`);
    }
...
async function main() {
  ...
  const backendReady = await waitForHttp(backendUrl, "backend");
  ...
  await writeFile(path.join(outputDir, "visual-design-smoke-latest.json"), `${JSON.stringify(result, null, 2)}\n`);

  if (failures.length > 0) {
    throw new Error(`Visual design smoke failed: ${failures.join("; ")}`);
  }
}
